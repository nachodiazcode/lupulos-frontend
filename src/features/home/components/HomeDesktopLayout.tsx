...
/* ── Home feed data ──
   Mock content removed: these start empty and each section renders its empty
   state. Wire to /api/* when the endpoints are ready. */
...
const tonightPlans: TonightPlan[] = [];
...
function Swimlane({ title, subtitle, emptyMessage, children }: { title: string; subtitle?: string; emptyMessage?: string; children: React.ReactNode }) {
  ...
  return (
    <section className="mb-4 w-full px-4">
      ...
      {isEmpty ? (
        <div
          className="flex items-center justify-center rounded-2xl border border-dashed py-10 px-4 text-center text-sm font-semibold"
  ...
  return (
    <div className="w-full flex flex-col pb-12">
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
        <Swimlane title="Tendencias de la Semana" subtitle="Las cervezas más valoradas por la comunidad" emptyMessage="Aún no hay cervezas en tendencia. Vuelve cuando la comunidad empiece a calificar.">
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
        <Swimlane title="Actividad Reciente" subtitle="¿Qué están tomando tus amigos?" emptyMessage="Todavía no hay actividad reciente. Sé el primero en compartir una cata.">
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}>
        <Swimlane title="Panoramas de esta Noche" subtitle="Eventos cerveceros que pasan hoy cerca tuyo" emptyMessage="No hay panoramas cargados por ahora. Atento a los próximos eventos.">
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }}>
        <Swimlane title="Noticias" subtitle="Lo que está pasando en el mundo cervecero chileno" emptyMessage="Sin noticias por ahora. Pronto habrá novedades del mundo cervecero.">
          {newsItems.map((news) => (
            <article
              key={news.id}
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
        <Swimlane title="Vlogs de Cerveceros" subtitle="Catas, recorridos y tutoriales de la comunidad" emptyMessage="Aún no hay vlogs publicados. Sube el primero desde tu perfil.">
          {vlogItems.map((vlog) => (
            <div
              key={vlog.id}
